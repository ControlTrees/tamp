...
checked     # object X as been check, the agnt knows which block it is
is
...
DecisionRule check {
  X
  { (block X) (holding X) (checked X)! }
  { (in_sight X) (checked X) komoCheck(X)=1. }
}
...
Rule {
  ...
  { (in_sight X)! (is X Y) (NOT_OBSERVABLE is X Y)!}
}
