...
def test_dist_3d_minimization():
    f0 = SquareDistance3D(cx=1, cy=1, cz=1, sx=1, sy=1, sz=0)
    f1 = SquareDistance3D(cx=1, cy=1, cz=1, sx=0, sy=1, sz=1)
...
def test_constrained_dist_3d():
    ...
    pb0 = ConstrainedProblem(f=SquareDistance3D(cx=1, cy=1, cz=1, sx=1, sy=1, sz=0), h=ProjX())
    pb1 = ConstrainedProblem(f= SquareDistance3D(cx=1, cy=1, cz=1, sx=0, sy=1, sz=1), h=ProjX())
...
def test_constrained_dist_3d_sphere():
    ...
    pb0 = ConstrainedProblem(f=SquareDistance3D(1, 1), h=h)
    pb1 = ConstrainedProblem(f= SquareDistance3D(1, 1), h=h)
    pb = ADMMProblem(pb0=pb0, pb1=pb1)
    solver = ADMMSolver(pb)
    x = solver.run(x0, observer=p)
